...
const EachRow = ({
  ...
  console.log(
    "Cipher Text :: in each row for control and uncontrol :: ",
    cipherPassword
  );
      ...
      <div className="flex text-sm w-full">
        ...
        >
          <input
            ...
            }  hover:cursor-pointer `} //or -left-7
          ></input>
        </span>
        <span
          className={` 
          flex-2 px-2 py-1 bg-pink-500 
          text-wrap min-w-0 
          border-y-1 border-l-1`}
        >
          {title}
        </span>
        <span className="flex-2 px-2 py-1 bg-lime-500 truncate min-w-0 border-y-1 border-l-1 text-center">
          {/* <-- Kept truncate for username, but added min-w-0 and removed fixed width */}
          {username}
        </span>

        {/* <-- Password column fix --> */}
        <span className="flex flex-[2.5] px-2 py-1 min-w-0 z-10  bg-violet-500 items-center justify-between border-l-1 border-y-1 ">
        ...
        <Link
          ...
          target="_blank"
          rel="noopener"
          className="flex-2 px-2 py-1 bg-sky-500 text-wrap min-w-0 border-y-1 border-l-1 hover:underline transition-all duration-300 break-words"
        >
        ...
        <span
          ...
          flex flex-2 px-2 py-1 bg-yellow-500 items-start text-wrap min-w-0 border-y-1 border-x-1`}
        >
